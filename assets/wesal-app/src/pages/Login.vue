        ...
        <div class="panoramic-card">
          ...
          <div class="card-content">
            ...
            <form @submit.prevent="handleLogin" class="panoramic-form" novalidate>
              <div class="form-row">
                <div class="input-nexus">
                  ...
                  <div class="input-wrapper">
                    <input v-model="email" type="email" placeholder="you@example.com" @input="isError = false" required />
              ...
              <div class="form-row">
                <div class="input-nexus">
                  ...
                  <div class="input-wrapper">
                    <input v-model="password" type="password" placeholder="Enter your password" @input="isError = false" required />
                    <div class="glowing-bar"></div>
                  </div>
                </div>
              </div>
            ...
            <div class="card-extra-links">
              ...
              <router-link to="/create-account" class="nexus-link accent">Create account</router-link>
            </div>
          </div>
        </div>
      </div>
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (!userRow) {
      triggerMsg('Invalid email or password.', true)
      return
    }
    ...
    if (!authRow || authRow.password !== password.value) {
      triggerMsg('Invalid email or password.', true)
      return
    }

    localStorage.setItem('logged_user_id', userRow.user_id)
    localStorage.setItem('logged_user_role', authRow.role)

    if (authRow.role === 'Support Agent') {
      router.push('/staff-dashboard')
    } else if (authRow.role === 'Customer') {
      router.push('/dashboard')
    } else {
      // role === 'Employee' — no dedicated page for this role yet
      triggerMsg('Employee dashboard is not set up yet.', true)
    }
  } catch (err) {
...
.link-sep { margin: 0 10px; color: #cbd5e1; }
